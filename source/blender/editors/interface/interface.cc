...
void block_draw(const bContext *C, Block *block)
{
  ...
  if (block->panel && ELEM(region->regiontype, RGN_TYPE_HUD, RGN_TYPE_TEMPORARY)) {
    /* Custom color for layout panels in popups (RGBA format).
     * Current values: R=0.2, G=0.3, B=0.33, A=0.05 (light blue-gray, very transparent).
     * You can adjust these values to change the background color. */
     float subpanel_backcolor[4]{0.2f, 0.3f, 0.33f, 0.05f};
    
    /* Alternative: Use theme color instead of hardcoded color */
